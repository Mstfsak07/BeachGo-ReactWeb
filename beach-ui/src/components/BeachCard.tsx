...
const FACILITY_LIST: FacilityItem[] = [
...
const BeachCard = memo(({ beach }: BeachCardProps) => {
  ...
  const activeFacilities = FACILITY_LIST.filter(({ key }) => Boolean(beach[key]));
  ...
  return (
    ...
    >
      ...
      <div className="px-3 pb-4 space-y-4">
        ...
        {beach.description && (
          <p className="text-slate-500 text-sm font-medium line-clamp-2 leading-relaxed">{beach.description}</p>
        )}

        {activeFacilities.length > 0 && (
          <div className="flex flex-wrap items-center gap-2">
            {activeFacilities.map(({ key, icon: Icon, color }) => (
