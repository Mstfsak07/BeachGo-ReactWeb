...
const BeachDetail = () => {
  ...
  const handleReservation = async (e) => {
    ...
    try {
      ...
      toast.success(`Rezervasyon BaĹąarÄąyla AlÄąndÄą! Takip Kodunuz: ${res.data.data.code}`);
  ...
  if (!beach) return <div className="text-center py-20 italic">Plaj bulunamadÄą.</div>;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
               {[
                 ...
                 { label: 'Doluluk', val: `%${beach.occupancyRate}`, sub: 'CanlÄą Durum', icon: Users, color: beach.occupancyRate > 80 ? 'text-red-500' : 'text-green-500' },
                 { label: 'Su SÄącaklÄąÄąÄą', val: '24Â°C', sub: 'Ä°deal', icon: Wind, color: 'text-blue-500' },
          ...
          <div className="space-y-6">
            <div className="card p-8 border-primary-50 ring-8 ring-primary-50/50 shadow-2xl">
               <h3 className="text-2xl font-black text-slate-800 mb-8 tracking-tighter">Hemen Yerini AyÄąrt</h3>
               <form onSubmit={handleReservation} className="space-y-6">
                  <div>
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2 block">Ĺzellikler</label>
                    <div className="grid grid-cols-1 gap-4">
                       <input 
                          type="text" className="input-field py-4 text-base" placeholder="Ä°sim Soyisim" required
               ...
               <p className="text-[10px] text-slate-400 mt-6 text-center font-medium italic">
                 Onay bekleyen talepler iĹąletmeci tarafÄąndan incelenmektedir.
               </p>
            </div>
          </div>
        </div>
