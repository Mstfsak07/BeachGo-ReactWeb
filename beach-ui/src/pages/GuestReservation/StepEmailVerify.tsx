...
type StepEmailVerifyProps = GuestReservationStepProps & {
  onNext: () => Promise<void> | void;
  onBack: () => void;
};

const StepEmailVerify = ({ formData, updateForm, onNext, onBack }: StepEmailVerifyProps) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => setCooldown((currentCooldown) => currentCooldown - 1), 1000);
    return () => clearInterval(timer);
  }, [cooldown]);

  useEffect(() => {
    if (lockout <= 0) return;
    const timer = setInterval(() => setLockout((currentLockout) => currentLockout - 1), 1000);
    return () => clearInterval(timer);
  }, [lockout]);
  ...
  const maskEmail = (email: string) => {
    ...
    return `${visiblePrefix}${'*'.repeat(Math.max(name.length - 2, 1))}@${domain}`;
  };
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (result?.verified) {
        updateForm({ emailVerified: true, otpCode: code });
        await onNext();
      } else {
        handleFailedAttempt();
      }
  ...
  return (
    ...
    >
      <div className="text-center">
        ...
        <h2 className="text-3xl font-black text-slate-900 tracking-tight mb-3">E-postanı Doğrula</h2>
        <p className="text-sm text-slate-500 font-medium leading-relaxed px-4">
          <span className="font-black text-slate-800 block mb-1 tracking-wide">{maskEmail(formData.email)}</span>
          adresine bir doğrulama kodu gönderdik. Lütfen aşağıdaki alana girin.
        </p>
      </div>
      ...
      <div className="space-y-6">
        ...
        {lockout > 0 && (
          <motion.div
            ...
            className="flex items-center justify-center gap-2 text-rose-600 font-bold text-sm bg-rose-50 py-3 rounded-xl border border-rose-100"
          >
      ...
      <div className="text-center">
        {cooldown > 0 ? (
          <p className="text-sm text-slate-500 font-medium flex items-center justify-center gap-2 bg-slate-50 py-3 rounded-xl border border-slate-100">
            <Clock size={16} className="text-slate-400" /> Tekrar göndermek için{' '}
            <span className="font-bold text-slate-700">{formatTime(cooldown)}</span>
          </p>
        ) : (
          <button
            type="button"
