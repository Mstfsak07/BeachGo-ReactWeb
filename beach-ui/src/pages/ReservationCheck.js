...
const ReservationCheck = () => {
  ...
  const getStatusBadge = (status) => {
    if (status === 'Approved' || status === 1) {
      return <span className="bg-emerald-100 text-emerald-600 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest text-center w-full block">OnaylandÄ±</span>;
    }
    ...
    if (status === 'Cancelled' || status === 3 || status === 'Rejected' || status === 2) {
      return <span className="bg-rose-100 text-rose-600 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest text-center w-full block">Ä°ptal / Red</span>;
    }
  ...
  const getPaymentBadge = (status) => {
    if (status === 'Paid') {
      return <span className="bg-emerald-100 text-emerald-600 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest text-center w-full block">Ã–dendi</span>;
    }
    if (status === 'Failed') {
      return <span className="bg-rose-100 text-rose-600 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest text-center w-full block">BaÅŸarÄ±sÄ±z</span>;
    }
    return <span className="bg-orange-100 text-orange-600 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest text-center w-full block">Ã–deme Bekliyor</span>;
  };
  ...
  const handleCheck = async (e) => {
    ...
    try {
      ...
      if (data && data.confirmationCode) {
        ...
        setMsg('ğŸ” Bu kod ile kayÄ±tlÄ± bir rezervasyon bulunamadÄ±.');
      }
    } catch (err) {
      if (err.response && err.response.status === 404) {
        setMsg('ğŸ” Bu kod ile kayÄ±tlÄ± bir rezervasyon bulunamadÄ±.');
      } else {
        setMsg('âŒ Sorgulama sÄ±rasÄ±nda bir hata oluÅŸtu. Kodun doÄŸru olduÄŸundan emin olun.');
      }
    } finally {
      setLoading(false);
    }
           ...
           <form onSubmit={handleCheck} className="flex flex-col md:flex-row gap-4">
              <div className="flex-grow">
                 ...
                 <input 
                    ...
                    placeholder="Ã–rn: ABC123" required
              ...
              <div className="flex items-end">
                 <button type="submit" disabled={loading} className="btn-primary w-full md:w-auto px-10 py-4 font-black uppercase text-sm tracking-widest disabled:opacity-50">
                    {loading ? 'SorgulanÄ±yor...' : 'Rezervasyonu Bul'}
                 </button>
              </div>
           </form>
        </div>
             ...
             <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                <div className="flex-grow">
                   <div className="flex items-center gap-3 mb-4">
                      ...
                      <h3 className="text-2xl font-black text-slate-800 tracking-tight">{reservation.beachName || "KonyaaltÄ± PlajÄ±"}</h3>
                   </div>
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">MÃ¼ÅŸteri</span>
                        <span className="font-bold text-slate-700">{reservation.guestName}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">KiÅŸi SayÄ±sÄ±</span>
                        <span className="font-bold text-slate-700">{reservation.personCount} KiÅŸi</span>
                      </div>
                      <div className="flex flex-col">
                   ...
                   {(reservation.status === 'Pending' || reservation.status === 'Waiting' || reservation.status === 0) && (
                     ...
                     >
                       {cancelLoading ? 'Ä°ptal Ediliyor...' : 'Rezervasyonu Ä°ptal Et'}
                     </button>
                   )}
                </div>
                <div className="flex flex-col gap-3 items-center min-w-[140px]">
                   ...
                   <div className="w-full bg-slate-50 p-3 rounded-xl border border-slate-100 flex flex-col items-center">
                     <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Ã–deme</span>
