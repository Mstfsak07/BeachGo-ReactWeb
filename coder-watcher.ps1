...
while ($true) {
    if (Test-Path $promptFile) {
        ...
        if (![string]::IsNullOrWhiteSpace($content)) {
            ...
            # chcp 65001 ile terminal utf-8 yapiliyor ve Aider sorunsuz calistiriliyor
            cmd.exe /c "chcp 65001 > nul & aider --model ollama_chat/qwen3:8b --yes-always --auto-commits --no-pretty --message-file ""$promptFile"" > ""$logFile"" 2>&1"
